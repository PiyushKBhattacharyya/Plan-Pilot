...
import { PlanProvider, PlanItem, PhaseItem } from './planProvider';
...
async function showPhaseDetails(phase: any) {
  const panel = vscode.window.createWebviewPanel(
    'phaseDetails',
    `Phase: ${phase.title}`,
    vscode.ViewColumn.One,
...
function generatePlanWebview(plan: Plan): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
function generatePhaseWebview(phase: any): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    ${phase.steps.sort((a: any, b: any) => a.order - b.order).map((step: any, index: number) => `
        <div class="step-item">
